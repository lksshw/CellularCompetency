...
def plot_all(rns, binned=False):
    ...
    if binned == True:
        ...
        ax2.fill_between(range(100), ms-l_ms, h_ms, alpha = 0.2, label = 'Range of Gene values (10 generation bins)', color='green')
        ...
        ax2.plot(range(1, config['RUNS']+1), m_comp, label = 'Gene value of Best individual')#, marker = 'x')
...
if __name__ == '__main__':
    ...
    for yu in tqdm(range(config['Loops'])):
        print('Running for Individual size: {}'.format(config['SIZE']))
    ...
    np.save('./weights/Hw_2genes_{}'.format(config['SIZE']), hw_runs)
    np.save('./weights/Comp_2genes_{}'.format(config['SIZE']), comp_runs)
    # np.save('./weights/fov_genetracker_2genes', fov_gene_tracker)
    # np.save('./weights/stress_genetracker_2genes', stress_gene_tracker)
    np.save('./weights/genetracker_bubble_{}'.format(config['SIZE']), bubble_genetracker)
    np.save('./weights/low_genetracker_bubble_{}'.format(config['SIZE']), low_bubble_genetracker)
    np.save('./weights/high_genetracker_bubble_{}'.format(config['SIZE']), high_bubble_genetracker)
        # np.save('./weights/genome_population', population_collections[0])
        # np.save('./weights/Competent_population', population_collections[1])

    np.save('./weights/genome_population_{}'.format(config['SIZE']), hw_ind_pop) 
    np.save('./weights/Competent_population_{}'.format(config['SIZE']), comp_ind_pop)
