...
class HelperFuncs():
  ...
  def swapvals(self, swaparray, pos):
      temp = swaparray[pos]
      swaparray[pos] = swaparray[pos+1]
      swaparray[pos+1] = temp
      ...
      while counter < swapsToUse and notswapped < self.config['SIZE']-1:
        ...
        for pos in poses_touse:
          ...
          if (individuals[n][pos] > individuals[n][pos+1]):
            ...
            self.swapvals(individuals[n], pos)
            counter +=1
            # individuals[n] = self.swapvals(individuals[n], pos) #swap and check if the swap increases fitness
            #if feasible:
  ...
  def crossover_withoutInterbreeding(self, organis):
    # Function to reproduce individuals in a population. After selection, population size is reduced. This function mimics reproduction to repopulate the population.
    # Inputs: The population to repopulate
    # Returns: The repopulated population
    ...
    while (len(organisms) < self.config['N_organisms']):  # Quit only when the current_size exceeds the number of organisms (N_organisms) allowed.
      hw_inds = [n for n,i in enumerate(organisms) if i[0]==-2]
      comp_inds = [n for n,i in enumerate(organisms) if i[0]==-1]

      roll = np.random.choice(['hw', 'comp'])
      checkFlag = False

      if roll == 'hw':
          if len(hw_inds) >=2:
              i, j = np.random.choice(hw_inds, 2, replace = False)
              checkFlag = True

          elif len(comp_inds) >=2 and len(hw_inds)<2:
              i, j = np.random.choice(comp_inds, 2, replace = False)
              checkFlag = True
          ...
          elif len(hw_inds) >=2 and len(comp_inds)<2:
              i, j = np.random.choice(hw_inds, 2, replace = False)
              checkFlag = True
...
if __name__=='__main__':
    ...
    # test = [7,8,1,4,6,4,8,3]
    test = np.random.randint(low =1, high =51, size = (1, 50))
    cfs = HelperFuncs(config)

    print(cfs.fitness(test[0]))
    q = cfs.bubble_sort(test)
    print(cfs.fitness(q[0]))
